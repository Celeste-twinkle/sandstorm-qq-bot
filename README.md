...
Local Qwen 支持 OneBot 图片段和 CQ 图片码。`@机器人` 时可以发送纯图片、图文消息，也可以先发送图片，再回复该图片并艾特机器人提问；缓存未命中时机器人会通过 OneBot `get_msg` 取回被引用的原图。每次模型请求最多保留上下文里最新的 10 张不同图片，图片会在本地执行类型、下载超时、单图大小和总大小检查后转成 OpenAI 兼容的 `image_url`。DeepSeek 回退链路保持文本模式，图片会转换为 `[图片]` 关联占位符。
