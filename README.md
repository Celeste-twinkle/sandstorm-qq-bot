...
所有联网路径都会根据 QQ 用户原始问题、模型搜索词和结果日期做相关性/时效性重排，并统一使用本地 Ollama 代理一键导入包中的 `web-evidence-research` 证据研究规则。搜索摘要只负责发现线索，关键结论必须读取正文；失败页面和搜索结果页不算证据，高风险信息还会限定地区、版本和日期。回答要求重要事实紧邻标注来源、明确区分事实与推断；如果来源不足、冲突或搜索质量差，会说明无法可靠确认并给出下一步验证方式。默认使用内嵌的 `open-websearch@2.1.11`，不需要单独启动服务，也不需要搜索 API Key；可通过 `OPEN_WEBSEARCH_ENGINES` 调整搜索引擎列表。
