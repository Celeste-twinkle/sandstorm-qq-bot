...
未 `@机器人` 的普通文字群聊可触发闲聊：即时闲聊会参考当前群最近上下文接话，默认最多取 `AMBIENT_CHAT_INSTANT_MAX_MESSAGES` 条；冷场闲聊会在 `AMBIENT_CHAT_IDLE_SECONDS` 秒无人继续发普通文字后，从群级最近上下文里按新到旧采集最多 `AMBIENT_CHAT_IDLE_MAX_MESSAGES` 条消息再回复。
