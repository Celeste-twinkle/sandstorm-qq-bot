...
const filteringHttpAgents = new Map();
const secureFilteringHttpsAgents = new Map();
const insecureFilteringHttpsAgents = new Map();
let insecureTrustedStaticHttpsAgent = null;
const proxyAgents = new Map();
function buildFakeIpAgentCacheKey() {
...
function getFilteringHttpAgent() {
...
function getFilteringHttpsAgent(allowInsecureTls) {
...
function getProxyAgent(proxyUrl, allowInsecureTls) {
...
function getInsecureTrustedStaticHttpsAgent() {
...
export function buildAxiosRequestOptions(options = {}) {
    const { allowInsecureTls = false, decompress, headers, maxBodyLength, maxContentLength, maxRedirects, params, responseType, timeout, trustedStaticHost = false, validateStatus } = options;
...
let axiosRequestImpl = (config) => axios.request(config);
...
export async function requestWithSafeRedirects(method, initialUrl, options = {}, urlLabel = 'Request URL') {
    ...
    for (let hops = 0; hops <= maxRedirects; hops++) {
        ...
        if (!validateStatus(response.status)) {
            throw new AxiosError(`Request failed with status code ${response.status}`, AxiosError.ERR_BAD_RESPONSE, response.config, response.request, response);
        }
        return response;
    }
