...
function winLock() {
    ...
    _winLock = {
        CreateFileW: kernel32.func('void * __stdcall CreateFileW(str16 lpFileName, uint32_t dwDesiredAccess, uint32_t dwShareMode, void *lpSecurityAttributes, uint32_t dwCreationDisposition, uint32_t dwFlagsAndAttributes, void *hTemplateFile)'),
        LockFileEx: kernel32.func('bool __stdcall LockFileEx(void *hFile, uint32_t dwFlags, uint32_t dwReserved, uint32_t nNumberOfBytesToLockLow, uint32_t nNumberOfBytesToLockHigh, _Inout_ OVERLAPPED *lpOverlapped)'),
        UnlockFileEx: kernel32.func('bool __stdcall UnlockFileEx(void *hFile, uint32_t dwReserved, uint32_t nNumberOfBytesToLockLow, uint32_t nNumberOfBytesToLockHigh, _Inout_ OVERLAPPED *lpOverlapped)'),
...
function winDesktop() {
    ...
    _winDesktop = {
        CreateDesktopW: user32.func('void * __stdcall CreateDesktopW(str16 lpszDesktop, void *lpszDevice, void *pDevmode, uint32_t dwFlags, uint32_t dwDesiredAccess, void *lpsa)'),
        CloseDesktop: user32.func('bool __stdcall CloseDesktop(void *hDesktop)'),
        CreateProcessW: kernel32.func('bool __stdcall CreateProcessW(str16 lpApplicationName, str16 lpCommandLine, void *lpProcessAttributes, void *lpThreadAttributes, bool bInheritHandles, uint32_t dwCreationFlags, void *lpEnvironment, str16 lpCurrentDirectory, _Inout_ STARTUPINFOW *lpStartupInfo, _Out_ PROCESS_INFORMATION *lpProcessInformation)'),
        DuplicateHandle: kernel32.func('bool __stdcall DuplicateHandle(void *hSourceProcessHandle, void *hSourceHandle, void *hTargetProcessHandle, _Out_ void **lpTargetHandle, uint32_t dwDesiredAccess, bool bInheritHandle, uint32_t dwOptions)'),
        ...
        CloseHandle: kernel32.func('bool __stdcall CloseHandle(void *hObject)'),
        CreatePipe: kernel32.func('bool __stdcall CreatePipe(_Out_ void **hReadPipe, _Out_ void **hWritePipe, SECURITY_ATTRIBUTES *lpPipeAttributes, uint32_t nSize)'),
        SetHandleInformation: kernel32.func('bool __stdcall SetHandleInformation(void *hObject, uint32_t dwMask, uint32_t dwFlags)'),
        PeekNamedPipe: kernel32.func('bool __stdcall PeekNamedPipe(void *hNamedPipe, void *lpBuffer, uint32_t nBufferSize, void *lpBytesRead, _Out_ uint32_t *lpTotalBytesAvail, void *lpBytesLeftThisMessage)'),
        ReadFile: kernel32.func('bool __stdcall ReadFile(void *hFile, _Out_ uint8_t *lpBuffer, uint32_t nNumberOfBytesToRead, _Out_ uint32_t *lpNumberOfBytesRead, void *lpOverlapped)'),
        STARTUPINFOW
    };
    return _winDesktop;
}
let _unixLock;
function getUnixLibcCandidates() {
 ...
 */
export function withNativeFileLock(lockFilePath, operation) {
 ...
 */
export async function withNativeFileLockAsync(lockFilePath, operation) {
    if (process.platform === 'win32') {
        return withWindowsFileLockAsync(lockFilePath, operation);
    }
    return withUnixFileLockAsync(lockFilePath, operation);
}
function withWindowsFileLock(lockFilePath, operation) {
    const w = winLock();
    const k = koffi();
    // 以共享方式打开锁文件（多进程可同时打开，靠 LockFileEx 互斥）
    const handle = w.CreateFileW(lockFilePath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, null, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, null);
...
function withUnixFileLock(lockFilePath, operation) {
    const u = unixLock();
...
async function withWindowsFileLockAsync(lockFilePath, operation) {
    const w = winLock();
    const k = koffi();
    const handle = w.CreateFileW(lockFilePath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, null, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, null);
    if (k.address(handle) === 0xffffffffffffffffn) {
...
async function withUnixFileLockAsync(lockFilePath, operation) {
    const u = unixLock();
    const fd = openSync(lockFilePath, 'a');
    if (u.flock(fd, LOCK_EX) !== 0) {
        closeSync(fd);
 ...
 * 进程崩溃后锁由 OS 自动释放。
 */
export function acquireNativeFileLock(lockFilePath) {
    if (process.platform === 'win32') {
 ...
 */
export function tryNativeFileLock(lockFilePath) {
    if (process.platform === 'win32') {
        return tryWindowsFileLock(lockFilePath);
    }
    return tryUnixFileLock(lockFilePath);
}
function acquireWindowsFileLock(lockFilePath) {
    const w = winLock();
    const k = koffi();
    const handle = w.CreateFileW(lockFilePath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, null, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, null);
    if (k.address(handle) === 0xffffffffffffffffn) {
...
function tryWindowsFileLock(lockFilePath) {
    const w = winLock();
    const k = koffi();
    const handle = w.CreateFileW(lockFilePath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, null, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, null);
    if (k.address(handle) === 0xffffffffffffffffn) {
...
function acquireUnixFileLock(lockFilePath) {
    const u = unixLock();
    const fd = openSync(lockFilePath, 'a');
    if (u.flock(fd, LOCK_EX) !== 0) {
        closeSync(fd);
...
function tryUnixFileLock(lockFilePath) {
    const u = unixLock();
    const fd = openSync(lockFilePath, 'a');
 ...
 */
export function launchProcessOnHiddenDesktopWithPipes(cmdLine, desktopName) {
    if (process.platform !== 'win32') {
    ...
    const browserPid = pi.dwProcessId;
    // 将桌面句柄复制到子进程
    ...
    if (hBrowserProc !== null && k.address(hBrowserProc) !== 0n) {
        ...
        w.DuplicateHandle(w.GetCurrentProcess(), hDesk, hBrowserProc, dupHandle, 0, false, DUPLICATE_SAME_ACCESS);
 ...
 */
export function peekNamedPipe(readHandle) {
    if (process.platform !== 'win32') {
 ...
 */
export function readNamedPipe(readHandle, maxBytes) {
    if (process.platform !== 'win32') {
 ...
 */
export function readNamedPipeAsync(readHandle, maxBytes) {
    if (process.platform !== 'win32') {
    ...
    return new Promise((resolve) => {
        w.ReadFile.async(readHandle, buf, maxBytes, bytesRead, null, (err, success) => {
            if (err || !success) {
                resolve(null);
                return;
            }
 ...
 * @returns 启动的进程 PID
 */
export function launchProcessOnHiddenDesktop(cmdLine, desktopName) {
    if (process.platform !== 'win32') {
