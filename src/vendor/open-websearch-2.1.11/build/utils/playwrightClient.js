...
let playwrightModuleSource = null;
let playwrightUnavailableMessage = null;
let hasEmittedPlaywrightUnavailableWarning = false;
let cachedBrowserPath = null;
let cachedLocalBrowserSession = null;
let localBrowserSessionPromise = null;
let cachedLocalBrowserSessionKey = null;
let cachedLocalBrowserSessionOptions = null;
...
function getPageLockFilePath(poolKey, pageTargetId) {
...
function getLocalBrowserSessionMode(headless, options) {
    if (options?.hideWindow) {
        return 'hidden-headed';
    }
    return headless ? 'headless' : 'headed';
}
function getBrowserPlaywrightPagePool(browser, options) {
...
async function withPoolAcquireLock(pool, operation) {
    while (pool.acquireLock) {
        await pool.acquireLock;
    }
    let releaseLock;
    pool.acquireLock = new Promise((resolve) => {
        releaseLock = resolve;
    });
    try {
...
async function getExistingContextPageWindowBounds(page) {
    try {
        const context = typeof page?.context === 'function' ? page.context() : null;
        if (!context || typeof context.newCDPSession !== 'function') {
...
async function isPopupLikePlaywrightPage(page) {
...
async function syncPoolWithReusableExistingContextPages(pool, context) {
...
async function createPooledPlaywrightPageEntry(browser, pool) {
...
function isRecoverableLocalBrowserSessionError(error) {
...
async function connectOverCdpOnly(playwright, endpoint, timeout) {
    return playwright.chromium.connectOverCDP(endpoint, { timeout });
}
async function closeConnectedCdpBrowser(browser, timeoutMs = 3000) {
...
function detachLaunchedChildProcess(child) {
    try {
...
async function probeLocalCdpReadiness(playwright, endpoint, probeTimeoutMs) {
    const browser = await connectOverCdpOnly(playwright, endpoint, probeTimeoutMs);
    try {
        await withOperationTimeout(browser.version(), probeTimeoutMs, `Timed out while probing local browser CDP readiness after ${probeTimeoutMs}ms`);
        return browser;
    }
    catch (error) {
...
async function connectOverCdpWhenReady(playwright, endpoint) {
...
async function recoverLocalBrowserSessionBrowser(browser) {
    ...
    const recoveredSession = await getOrCreateLocalBrowserSession(playwright, cachedLocalBrowserSessionOptions.headless, cachedLocalBrowserSessionOptions.launchArgs, cachedLocalBrowserSessionOptions.options);
    return recoveredSession.browser;
}
async function acquirePooledPlaywrightPageOnce(browser, options) {
...
export async function acquirePooledPlaywrightPage(browser, options) {
    try {
...
function normalizeLoadedPlaywrightModule(loaded) {
    if (loaded?.chromium) {
        return loaded;
    }
...
function buildLocalSessionKey(headless, launchArgs, options) {
 ...
 */
function buildBrowserDomainKey(mode) {
    if (mode === 'headed') {
...
function getBrowserDomainHash(domainKey) {
    return createHash('sha1').update(domainKey).digest('hex');
}
function getBrowserDomainLockFilePathByHash(domainHash) {
    mkdirSync(CROSS_PROCESS_BROWSER_SESSION_LOCK_DIR, { recursive: true });
    return path.join(CROSS_PROCESS_BROWSER_SESSION_LOCK_DIR, `domain-${domainHash}.lock`);
}
function getBrowserDomainLockFilePath(domainKey) {
    return getBrowserDomainLockFilePathByHash(getBrowserDomainHash(domainKey));
}
function getLocalBrowserSessionModeFromDomainKey(domainKey) {
...
function getBrowserDomainMetadataPath(domainKey) {
    mkdirSync(CROSS_PROCESS_BROWSER_SESSION_LOCK_DIR, { recursive: true });
    const sessionMode = getLocalBrowserSessionModeFromDomainKey(domainKey);
    return path.join(CROSS_PROCESS_BROWSER_SESSION_LOCK_DIR, `${LOCAL_BROWSER_DOMAIN_METADATA_PREFIX}${sessionMode}-${getBrowserDomainHash(domainKey)}.json`);
}
function listBrowserDomainMetadataEntries() {
    try {
        mkdirSync(CROSS_PROCESS_BROWSER_SESSION_LOCK_DIR, { recursive: true });
        const metadataFilePattern = new RegExp(`^${LOCAL_BROWSER_DOMAIN_METADATA_PREFIX}(headed|headless|hidden-headed)-([a-f0-9]+)\\.json$`, 'u');
...
function normalizeBrowserDomainMetadata(parsed, sessionMode, metadataPath, domainKey) {
    if (typeof parsed.tempDir !== 'string' || parsed.tempDir.length === 0)
        return null;
    return {
        domainKey,
        metadataPath,
        ...
        clientPids: shouldTrackLocalBrowserSessionClients(sessionMode) && Array.isArray(parsed.clientPids)
            ? parsed.clientPids.filter((pid) => Number.isInteger(pid) && pid > 0)
            : []
    };
}
function readBrowserDomainMetadataFromPath(metadataPath, sessionMode, domainKey) {
    try {
        return normalizeBrowserDomainMetadata(JSON.parse(readFileSync(metadataPath, 'utf8')), sessionMode, metadataPath, domainKey);
    }
    catch {
        return null;
    }
}
function readBrowserDomainMetadata(domainKey) {
    return readBrowserDomainMetadataFromPath(getBrowserDomainMetadataPath(domainKey), getLocalBrowserSessionModeFromDomainKey(domainKey), domainKey);
}
function serializeBrowserDomainMetadata(metadata) {
...
function writeBrowserDomainMetadata(metadata) {
    ...
    if (!metadataPath)
        return;
    try {
        writeFileSync(metadataPath, JSON.stringify(serializeBrowserDomainMetadata(metadata), null, 2), 'utf8');
    }
    catch {
        // metadata 写入失败只影响跨进程复用，当前进程仍然可以继续使用已连接的浏览器。
    }
}
function readBrowserDomainMetadataTempDirFromPath(metadataPath) {
    try {
...
function clearBrowserDomainMetadataFromPath(metadataPath, tempDir) {
    if (tempDir) {
...
function clearBrowserDomainMetadata(domainKey, tempDir) {
...
function normalizeActiveClientPids(clientPids) {
    return [...new Set(clientPids.filter((pid) => processExists(pid)))];
}
function shouldTrackLocalBrowserSessionClients(sessionMode) {
    return sessionMode !== 'headed';
}
function registerLocalBrowserSessionClient(metadata, pid = process.pid) {
    if (!shouldTrackLocalBrowserSessionClients(metadata.sessionMode)) {
...
function unregisterLocalBrowserSessionClient(metadata, pid = process.pid) {
    if (!shouldTrackLocalBrowserSessionClients(metadata.sessionMode)) {
...
function createProcessInspectionTimeoutError(message, cause) {
    const error = new Error(message);
    error.name = 'LocalBrowserProcessInspectionTimeoutError';
    error.cause = cause;
    return error;
}
function isProcessInspectionTimeoutError(error) {
...
function getProcessCommandLine(pid) {
    ...
    try {
        ...
        if (process.platform === 'win32' && isExecTimeoutError(error)) {
            throw createProcessInspectionTimeoutError(`PowerShell timed out while querying command line for PID ${pid}`, error);
        }
        return null;
    }
}
function getLocalBrowserDebugPortCommandLineFragment(debugPort) {
    return isValidLocalBrowserDebugPort(debugPort)
        ? `--remote-debugging-port=${debugPort}`
        : '--remote-debugging-port=';
}
function isValidLocalBrowserDebugPort(debugPort) {
    return typeof debugPort === 'number' && Number.isInteger(debugPort) && debugPort > 0;
}
function extractLocalBrowserDebugPortFromCommandLine(commandLine) {
...
function commandLineMatchesLocalBrowserCandidate(commandLine, tempDir, debugPort) {
...
function createLocalBrowserCandidateFromCommandLine(pid, commandLine, tempDir, debugPort) {
...
function getLocalBrowserCandidateFromPid(pid, tempDir, debugPort) {
...
function listLocalBrowserCandidatesByTempDir(tempDir, debugPort) {
    ...
    if (process.platform !== 'win32') {
        try {
            ...
            return raw.split(/\r?\n/u)
                ...
                .filter((candidate) => candidate !== null && processExists(candidate.pid));
        }
        catch {
            return [];
        }
    ...
    try {
        ...
        const raw = execFileSync('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', script], { encoding: 'utf8', windowsHide: true, timeout: 5000 }).trim();
        if (!raw) {
            return [];
        }
        ...
        if (isExecTimeoutError(error)) {
            throw createProcessInspectionTimeoutError(`PowerShell timed out while enumerating local browser candidate processes for ${tempDir}`, error);
        }
        return [];
    }
}
function resolveLocalBrowserCandidate(preferredPid, tempDir, debugPort) {
    if (preferredPid) {
...
function resolveLocalBrowserCandidatePid(preferredPid, tempDir, debugPort) {
    return resolveLocalBrowserCandidate(preferredPid, tempDir, debugPort)?.pid;
}
function requireLocalBrowserCandidatePid(preferredPid, tempDir, debugPort, context) {
...
function quoteWindowsCommandLineArg(arg) {
    if (arg.length === 0) {
        return '""';
    }
    if (!/[\s"]/u.test(arg)) {
...
function trackLocalBrowserSessionClientForReuse(metadata, pid = process.pid) {
    if (!shouldTrackLocalBrowserSessionClients(metadata.sessionMode)) {
        return { ...metadata, clientPids: [] };
    }
    return registerLocalBrowserSessionClient(metadata, pid);
}
function cleanupStaleLocalBrowserSessions() {
    ...
    for (const { domainHash, metadataPath, sessionMode } of entries) {
        ...
        try {
            ...
            const browserCandidatePid = resolveLocalBrowserCandidatePid(normalizedMetadata.browserPid, normalizedMetadata.tempDir, normalizedMetadata.debugPort);
...
async function connectToLocalDebugBrowser(playwright, port) {
 ...
 */
async function waitForBrowserReadyViaStdout(source, timeoutMs = 30000) {
    let accumulated = '';
    if (source.type === 'pipe') {
 ...
 */
async function tryReusePersistedLocalBrowserSession(playwright, domainKey, sessionKey) {
    ...
    if (!reusableBrowserCandidate) {
        ...
        catch { /* metadata 指向的会话目录已经不可复用，清理失败不影响新建。 */ }
        return null;
    }
...
async function closeLocalBrowserSession(session) {
...
function createForceKill(browserPid, tempDir, browser, domainKey) {
    return () => {
        try {
            browser?.disconnect?.();
        }
...
function createLocalBrowserLaunchError(message, browserPid, cause) {
    const error = new Error(message);
...
function getLocalBrowserLaunchErrorPid(error) {
    if (!error || typeof error !== 'object') {
        return undefined;
    }
    const browserPid = error.browserPid;
    return Number.isInteger(browserPid) && browserPid > 0
        ? browserPid
        : undefined;
}
function resolveLaunchFailureBrowserPid(preferredPid, tempDir, debugPort, error) {
...
function registerLocalBrowserCleanup() {
...
async function connectLaunchedLocalBrowserSession(playwright, endpoint, preferredPid, tempDir, debugPort, context) {
    let browser;
    let verifiedBrowserPid;
    try {
        browser = await connectOverCdpWhenReady(playwright, endpoint);
    }
    catch (error) {
        const candidatePid = resolveLocalBrowserCandidatePid(preferredPid, tempDir, debugPort);
        if (!candidatePid) {
            throw createLocalBrowserLaunchError(`${context}: CDP connection failed and no local browser process matched tempDir/debugPort: ${getErrorMessage(error)}`, undefined, error);
        }
        ...
        try {
            ...
            throw createLocalBrowserLaunchError(`${context}: CDP connection failed for verified local browser process PID ${candidatePid}: ${getErrorMessage(retryError)}`, candidatePid, retryError);
        }
    }
    try {
        ...
        await closeConnectedCdpBrowser(browser);
        throw createLocalBrowserLaunchError(error instanceof Error ? error.message : `${context}: local browser process verification failed`, verifiedBrowserPid, error);
    }
}
async function launchHiddenDesktopBrowser(playwright, sessionKey, domainKey, launchArgs) {
    ...
    if (process.platform === 'win32') {
        ...
        const endpoint = `http://127.0.0.1:${port}`;
        try {
            const connectedSession = await connectLaunchedLocalBrowserSession(playwright, endpoint, browserPid, tempDir, port, 'Hidden local browser launch');
    ...
    const endpoint = `http://127.0.0.1:${port}`;
    try {
        const connectedSession = await connectLaunchedLocalBrowserSession(playwright, endpoint, browserPid, tempDir, port, 'Hidden desktop local browser launch');
        browserPid = connectedSession.browserPid;
        const browser = connectedSession.browser;
        writeBrowserDomainMetadata({
            domainKey,
...
async function launchStandardLocalBrowser(playwright, sessionKey, domainKey, headless, launchArgs) {
    if (process.platform === 'win32') {
        const browserPath = getLocalBrowserExecutablePath();
        const tempDir = mkdtempSync(path.join(tmpdir(), 'mcp-search-'));
        const port = await findFreePort();
        const args = buildLocalBrowserProcessArgs(port, tempDir, launchArgs, headless);
        const sessionMode = headless ? 'headless' : 'headed';
        ...
        try {
            const connectedSession = await connectLaunchedLocalBrowserSession(playwright, endpoint, child.pid, tempDir, port, 'Local browser launch');
            browserPid = connectedSession.browserPid;
            const browser = connectedSession.browser;
            writeBrowserDomainMetadata({
                domainKey,
...
async function destroyCachedLocalBrowserSession() {
...
export async function shutdownLocalPlaywrightBrowserSessions() {
    if (cachedLocalBrowserSession) {
        try {
            await closeLocalBrowserSession(cachedLocalBrowserSession);
        }
...
async function getOrCreateLocalBrowserSession(playwright, headless, launchArgs, options) {
...
function getPlaywrightModuleCandidates() {
    const candidates = [];
    const seenSpecifiers = new Set();
    const pushCandidate = (label, specifier) => {
...
export function getPlaywrightModuleSource() {
    return playwrightModuleSource;
}
function emitPlaywrightUnavailableWarning(options) {
...
export async function loadPlaywrightClient(options) {
...
export async function openPlaywrightBrowser(headless, launchArgs = [], options) {
    const playwright = await loadPlaywrightClient();
    if (!playwright) {
