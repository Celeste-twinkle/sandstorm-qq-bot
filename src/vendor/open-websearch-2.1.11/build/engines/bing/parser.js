...
function decodeBingRedirectTarget(url) {
...
function extractTitle($, element, fallbackUrl, index) {
    const candidateTitle = normalizeWhitespace(element.find('h2 a').first().text() ||
...
function extractDescription(element, title) {
    const directSnippet = normalizeWhitespace(element.find('.b_caption p').first().text() ||
...
function extractSource(element, url) {
    const sourceText = normalizeWhitespace(element.find('.b_tpcn').first().text() ||
...
function collectFallbackLinks($, limit, seenUrls, results) {
    const linkContainers = $('#b_results a[href], #b_topw a[href], .b_algo a[href], .b_ans a[href]');
    linkContainers.each((index, element) => {
...
export function parseBingSearchResults(htmlContent, limit) {
