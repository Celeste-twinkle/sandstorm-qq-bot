...
export function hasSiteOperator(query) {
    return /(^|\s)site:[^\s]+/i.test(query);
}
export function shouldSuggestRemovingSiteOperator(query, error) {
...
function buildBingSearchUrl(query, pageNumber) {
...
function analyzeBlockedPage(html) {
...
function buildDefaultBrowserLaunchArgs(hideWindow) {
    const args = [
        '--no-sandbox',
...
function buildWindowsBrowserLaunchArgs(hideWindow) {
...
function buildBrowserLaunchArgs(hideWindow, platform = process.platform) {
    return platform === 'win32'
        ? buildWindowsBrowserLaunchArgs(hideWindow)
        : buildDefaultBrowserLaunchArgs(hideWindow);
}
export function __buildBingBrowserLaunchArgsForTests(hideWindow, platform) {
...
async function preparePlaywrightPage(page) {
...
async function waitForBingSearchInputValue(page, expectedValue) {
    await page.waitForFunction(({ selectors, value }) => {
...
async function waitForAnyDeterministicSignal(signals, timeoutMs) {
    if (signals.length === 0) {
        return false;
    }
...
function normalizeBingQueryForUrl(query) {
    return query.trim().replace(/\s+/g, ' ').toLowerCase();
}
function doesBingUrlMatchQuery(url, query) {
    try {
...
async function waitForBingQueryNavigation(page, previousUrl, query) {
...
async function waitForBingNextPageNavigation(page, previousUrl) {
...
async function submitBingSearchFromCurrentPage(page, searchInput, previousUrl, query) {
...
async function findBingSearchInput(page) {
...
async function waitForBingSearchInput(page) {
...
async function openBingAndSearch(page, query) {
    ...
    if (!playwrightAvailabilityPromise) {
        playwrightAvailabilityPromise = (async () => {
            ...
            try {
                ...
                const session = await openPlaywrightBrowser(effectiveHeadless, buildBrowserLaunchArgs(shouldUseHiddenHeadedBingBrowser()), { hideWindow: shouldUseHiddenHeadedBingBrowser() });
...
async function searchBingWithPlaywright(query, limit) {
...
export async function searchBing(query, limit, options) {
